...
public class SoundDetection: ObservableObject {
    ...
    public func start(_ volumeThreshold: Int = 60) {
        ...
        tapNode.installTap(
            ...
            block: {
                (buffer, time) in
                self.startSoundDetection(from: buffer, volumeThreshold)
            })
    ...
    func startSoundDetection(from buffer: AVAudioPCMBuffer, _ volumeThreshold: Int = 60) {
        let amplitude = getMaxAmplitude(from: buffer)
        let value = calculateValue(from: amplitude)
        detectSound(volumeValue: value, volumeThreshold)
    }
